...
public class EntityService {
	...
	public List<Person> returnPeople(
		...
		String queryAbfrage = 	"select p from Person p where " + 
								"((:surname is null) or (p.surname = :surname)) and "+ 
								"((:forename is null) or (p.forename = :forename)) and " + 
								"((:email is null) or (p.email = :email))";

		final EntityManager radioManager = RestJpaLifecycleProvider.entityManager("radio");
		TypedQuery<Person> query = radioManager.createQuery(queryAbfrage, Person.class);
		
		query.setParameter("surname", surname);
		query.setParameter("forename", forename);
		query.setParameter("email", email);

		List<Person> allPersons = query
				.getResultList()
				.stream()
				.sorted(Comparator.comparing(Person::getSurname)
				.thenComparing(Person::getForename)
	...
	public long createOrModifyPerson (
		...
		if (insert) { // neue Person erstellen mit default Avatar
			...
			if (defaultAvatar == null) throw new IllegalStateException(); // ErrorCode 500
			person = new Person(defaultAvatar);
		} else { // Person, die bereits in DB existiert updaten
			person = radioManager.find(Person.class, personTemplate.getIdentity()); // Person aus DB suchen
			if (person == null)	throw new ClientErrorException(Status.NOT_FOUND); // code 404															// Not FOund
		}
	
		if(personTemplate.getGroup() != USER && person.getGroup() == USER) throw new ClientErrorException(FORBIDDEN);
		...
		if (insert) {
			...
			radioManager.flush(); // für alle veränderten Objekte im 1st Level Cache
		}
	...
	public long createOrModifyAlbum(
		...
		if(insert){
			final Document defaultCover = radioManager.find(Document.class, 1L); // sucht Identity mit der Identity 1
			if(defaultCover == null)
				throw new IllegalStateException(); 
			album = new Album(defaultCover);
		} else {
	...
	public Collection<Track> queryTracks(
		...
		String queryAbfrage = "select t.identity from Track as t where "
		...
		final EntityManager radioManager = RestJpaLifecycleProvider.entityManager("radio");
		final TypedQuery<Long> query = radioManager.createQuery(queryAbfrage, Long.class);
		if (resultOffset > 0) query.setFirstResult(resultOffset);	
		if (resultLimit > 0) query.setMaxResults(resultLimit);
		...
		for (final long id : trackReferences) {
			...
			if (track != null) resultTracks.add(track);
		}
	...
	public long createOrModifyTracks(
			...
			@HeaderParam(REQUESTER_IDENTITY) @Positive final long requesterIdentity,
			@QueryParam("Recording-Reference") final Long recordingReference,
			@QueryParam("Album-Reference") final Long albumReference,
			@QueryParam("Owner-Reference") final Long ownerReference
	){	
		final EntityManager radioManager = RestJpaLifecycleProvider.entityManager("radio");
		final Person requester = radioManager.find(Person.class,requesterIdentity);
		...
		if(insert){
			final Document defaultAvatar = radioManager.find(Document.class, 1L);
			if(defaultAvatar == null) throw new ClientErrorException(NOT_FOUND);
			track = new Track(defaultAvatar); /////nach default gucken
		} else{
		...
		if (ownerReference != null) {
			final Person owner = radioManager.find(Person.class, ownerReference);
			if (owner == null) throw new ClientErrorException(NOT_FOUND);
		...
		// Enitäten, die die Spiegelseite der Relationen beinhalten aus dem 2nd Level Cache löschen
		...
		if (ownerReference != null) cache.evict(Person.class, ownerReference);
	...
	/**
	 * POST /albums: Creates or updates an album from template data within the HTTP
	 * request body in application/json format. The requesting user must be an ADMIN, if not
	 * this operation shall be forbidden. It creates a new album if the given template's identity
	 * is zero, otherwise it updates the corresponding album with the given data. Optionally, a
	 * new cover reference may be passed using the query parameter "coverReference".
	 ...
	 */
	 ...
	 @Consumes(WILDCARD)
	 @Produces(TEXT_PLAIN)
	 public long createOrModifyDocument(
